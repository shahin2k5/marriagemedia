...
<form action="{{route('admin.createEducationForAdmin', $id)}}" method="post" enctype="multipart/form-data">
  {!! csrf_field() !!}
...
<div class="row">
  <div class="col-md-6">
    ...
    <select class="form-control" name="optStatus">
      <option value="{{$profileInfo->profile_status or ''}}">{{$profileInfo->profile_status or 'SELECT'}}</option>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $("#optCountry").on('change', function(e){
      ...
      // alert('yes');
      $.ajax({
        url:"{{route('admin.showCityForAdmin')}}",
        method:"GET",
