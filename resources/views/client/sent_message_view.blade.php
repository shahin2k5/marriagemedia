...
<li><a href="{{route('client.showSentMessage')}}"> Sent</a></li>
...
<a href="{{route('client.showSentMessage')}}" class="btn btn-info btn-sm pull-right"><i class="fa fa-reply"></i></a>
</div><!-- /.box-header -->
<div class="container-fluid">
  <div class="row">
    ...
    <div class="col-md-4">
      @if($messageView->ReceiverProfile->photo!="")
        <img src="{{asset('./profile/'.$messageView->ReceiverProfile->photo)}}" class="img-responsive" width="100%" />
      @else
...
@if(!empty($assignPackage))
    ...
    <form action="{{route('client.sendMessageByClient')}}" method="post">
      {!! csrf_field() !!}
    <div class="box-body">
    <input type="hidden" name="txtReiverId" value="{{$messageView->ReceiverProfile->id}}" />
